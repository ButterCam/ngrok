...
func newClientModel(config *Configuration, ctl mvc.Controller) *ClientModel {
	...
	// configure TLS
	if !config.NoTls {
		m.Info("Trusting root CAs: %v", config.RootCerts)
		m.Info("Load client cert: %s, %s", config.ClientCert, config.ClientKey)
		var err error
		if m.tlsConfig, err = LoadTLSConfig(config.RootCerts, config.ClientCert, config.ClientKey); err != nil {
			panic(err)
		}
